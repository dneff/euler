...
def xor_translate(cipher,key):
  answer = []
  for i in range(0,len(cipher)):
    key_position = (i % len(key))
    answer.append(chr(cipher[i] ^ ord(key[key_position])))
  return ('').join(answer)

  return answer

fh = open('./p059-cipher1.txt','r')

cipher = map(string.strip,fh.readlines())
cipher = [int(x) for x in cipher[0].split(',')]

solution = ''

for a in map(chr, range(97,123)):
  for b in map(chr, range(97,123)):
    for c in map(chr, range(97,123)):
      passphrase = [a, b, c]
      candidate = xor_translate(cipher,passphrase)
      if " the " in candidate.lower():
        solution = candidate

print 'solution: ',sum([ord(x) for x in list(solution)])
